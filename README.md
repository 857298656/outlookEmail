...
**手动刷新**：邮箱页可以刷新当前账号或全部账号；设置页配置每个文件夹拉取的最近邮件数。导入账号或保存 OAuth 账号后会执行一次上限为 1000 封/文件夹的初始化刷新。

后续刷新会先与 SQLite 缓存比较：Graph 只为新 message id 下载完整正文和附件元数据；IMAP 只为新 UID 下载完整 MIME，已缓存 UID 仅同步轻量已读标记。IMAP `UIDVALIDITY` 变化时会安全重建对应文件夹的最近缓存。
