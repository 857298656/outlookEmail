...
**手动刷新**：邮箱页可以刷新当前账号或全部账号；设置页配置每个文件夹拉取的最近邮件数，上限为 2000 封/文件夹。导入账号或保存 OAuth 账号后会执行一次同上限的初始化刷新。

后续刷新会先与 SQLite 缓存比较：Graph 只为新 message id 下载完整正文和附件元数据；IMAP 只为新 UID 下载完整 MIME，已缓存 UID 仅同步轻量已读标记。旧缓存第一次记录 IMAP `UIDVALIDITY` 时不会被清空；确认发生变化后，也只有完整拉取替换窗口成功才会原子重建，查询或写入失败会保留原缓存。部分服务商的 `UID SEARCH` 返回异常，或 `UID FETCH` 成功但正文不完整时，会自动回退普通序号搜索和拉取。
