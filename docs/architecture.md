...
- IMAP `UIDVALIDITY` is stored per application folder in `accounts.provider_sync_state`. A changed value, or the first post-upgrade refresh of an existing cache without saved validity, resets that folder's local cache before safely refilling the recent window.
