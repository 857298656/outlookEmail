...
- Settings persistence, local retention cleanup, and configurable manual mail refresh limits
...
- Graph uses Microsoft OAuth v2 and Graph `/me/mailFolders/{folder}/messages` calls. OAuth auth URL generation and token exchange accept `graph` and Outlook `imap` providers only. After the initial cache fill, refresh lists only lightweight message ids/read state in the recent window and downloads full bodies plus attachment metadata only for ids absent from SQLite.
...
- IMAP uses TLS login, UID search/fetch, and MIME parsing. Outlook IMAP OAuth accounts authenticate with XOAUTH2 using Microsoft refresh tokens. Refresh compares recent UIDs with SQLite, downloads full MIME only for unseen UIDs, and uses a lightweight `FLAGS` fetch for cached UIDs.
...
- IMAP `UIDVALIDITY` is stored per application folder in `accounts.provider_sync_state`. The first post-upgrade refresh records the value without clearing an existing cache. A later confirmed change rebuilds the recent folder window only after the complete replacement set has been fetched, and the cache/state write is applied atomically so failures preserve the prior cache. If `UID SEARCH` fails or returns no UIDs for a non-empty mailbox, or `UID FETCH` returns fewer message bodies than requested, refresh falls back to sequence search/fetch while still storing stable UIDs.
...
- Refresh avoids retransferring full content already cached in SQLite. The configured limit remains a recent-message discovery window per folder, not a forced redownload count.
...
- Mail refresh is manual; there is no background refresh thread or periodic refresh setting.
- The mailbox view refreshes the current account or walks all displayed accounts sequentially through `run_refresh_job`.
- `manual_refresh_top` controls the recent-message limit per folder. Account import and OAuth account creation use the provider maximum for their initial refresh.
