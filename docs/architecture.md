...
- Provider foundation work has started with registry-backed import detection, provider labels, provider filtering, batch import provider/credential previews, IMAP presets for QQ/163, provider-specific credential hints, centralized adapter routing, Gmail OAuth account save support, and Gmail API first-read sync.
