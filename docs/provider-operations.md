...
person@gmail.com----gmail-app-password
```
...
| 现象 | 优先检查 | 处理方式 |
| --- | --- | --- |
| 账号显示“缺少凭据” | provider 是否正确、OAuth refresh token 或 IMAP 密码是否存在 | Outlook 重新授权并保存；Gmail/QQ/163/Custom IMAP 在账号设置里填入 IMAP 应用密码、授权码或授权密码。 |
...
Gmail IMAP 登录失败、QQ 授权码错误，以及 163 客户端授权密码/网页登录密码错误都会归类为 `auth`，刷新失败汇总和重试退避会按凭据问题处理。
刷新管理页会在失败服务商汇总和失败账号错误列中显示对应处理建议，优先提示重新授权、应用专用密码、授权码/授权密码、IMAP 开启或代理网络检查。
...
- 桌面应用已完成本地密码解锁，数据库可写。
- Outlook OAuth callback URL 与应用设置一致。
- Gmail/QQ/163 已在网页端开启 IMAP，并生成应用专用密码、一次性授权码或客户端授权密码。
- 如需代理，账号主代理和备用代理链已配置。
...
1. 在 Gmail 设置中开启 IMAP。
2. 在 Google 账号中生成应用专用密码。
3. 导入或创建账号，provider 选择 Gmail，IMAP host 应为 `imap.gmail.com`，端口 993。
4. 使用应用专用密码作为 IMAP 密码刷新 Inbox。
5. 确认邮件列表、正文预览、HTML 正文、附件元数据可见。
6. 下载至少一个 Gmail 附件，确认文件名、MIME 类型和大小合理。
7. 执行标记已读、标记未读，确认本地状态更新且没有远端失败面板。
8. 删除一封测试邮件，确认远端操作失败时会进入重试队列并在 UI 可见；成功时缓存同步更新。
9. 人为改错应用专用密码，确认刷新失败可进入失败汇总和重试队列。
...
3. 账号管理表和刷新管理表的凭据列应显示 OAuth、应用专用密码、授权码/授权密码或缺少配置详情。
...
- `src/lib/mailPreview.test.ts` 覆盖邮件列表/预览摘要的 HTML/CSS 剥离与空预览回退。
- `src/lib/importParser.test.ts` 覆盖批量导入 provider 字段、域名识别和 legacy 行格式。
- `src/lib/providerRegistry.test.ts` 覆盖前端 provider capability metadata、Gmail/QQ/163 IMAP 文件夹能力。
...
- `db::project_tests::import_accounts_detects_mail_provider_presets` 覆盖导入后 QQ/163/Gmail provider、account_type、IMAP host/port 归一化，以及账号列表的 IMAP 密码就绪布尔字段。
- Gmail 当前默认走 IMAP；回归重点是 provider 归一化、IMAP preset、应用专用密码就绪检查和 IMAP 刷新/附件路径。
- `providers::tests::classifies_imap_special_use_and_common_mailbox_names` 覆盖 IMAP special-use 和常见中文文件夹名映射。
- `providers::tests::build_imap_select_variants_includes_quoted_and_unquoted_names` 覆盖 IMAP SELECT 文件夹名的引号/去引号重试变体。
