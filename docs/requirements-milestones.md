...
继续按“桌面版够用，优先补功能”推进。当前桌面优先级中的 P0/P1/P2 项已补齐；M9 已启动并完成首批 Provider Foundation、Gmail OAuth/API 同步基础、Gmail 远端操作、Gmail `historyId` 增量同步、QQ IMAP preset/提示基础和 163 IMAP preset/中文文件夹映射基础。下一步建议继续 P9.5：Provider UX and Batch Operations；同时在具备真实 Gmail/QQ/163 测试账号时补做人工验收。Web 服务和浏览器扩展暂不处理。
