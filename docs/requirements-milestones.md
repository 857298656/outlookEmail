...
- 优先还原邮箱管理、邮件同步、本地导出分享和临时邮箱核心能力。
- 项目账号池、转发、WebDAV 备份、重试队列和自动化仪表盘已移出当前桌面范围。
...
> 本节保留早期需求演进记录，不作为当前验收范围；其中账号标签、项目账号池、转发、WebDAV、重试队列和自动化相关条目已经移除。当前实现与验收范围以 README、架构文档、M8 当前范围说明和「当前推荐下一步」为准。
...
- 邮件正文 HTML 通过清理和沙箱 iframe 安全渲染，不插入主应用 DOM 执行；其中 HTTP(S) 链接由桌面端接管并在系统默认浏览器打开。
...
- 已交付：临时邮箱列表、生成/分块导入、导入进度、消息刷新与 SQLite 缓存、删除、Cloudflare 通道设置。
...
- 目标：把当前本地桌面范围从可用推进到长期稳定。
- 已交付：SQLite WAL、本地密码派生加密、缓存清理和审计记录。
- 已交付：账号刷新状态、服务商错误归类和失败结果展示。
- 已交付：邮件 HTML 沙箱渲染、导出转义和本地凭据二次确认。
- 已交付：前后端回归测试与本地 workflow 级测试。
- 已移出当前范围：转发、WebDAV 备份、重试队列、自动化仪表盘和项目账号池。
- 风险点：真实邮件服务商和临时邮箱服务仍需人工验收。
