...
- 剩余增强：IMAP 附件下载、HTML 安全渲染、高级搜索排序、远端失败重试队列。

### M7：分享、导出和外部集成，还未完成

- 目标：补齐分享导出和脚本集成能力。
- 交付物：邮件 HTML 导出、分享链接、本地 HTTP API、项目池导出。
- 风险点：分享功能需要保持本地安全边界，避免意外暴露敏感邮件内容。

### M8：生产级稳定性，还未完成

- 目标：把功能从可用推进到长期稳定。
- 交付物：重试队列、备份恢复、调度仪表盘、代理故障切换、端到端测试。
- 风险点：自动化任务会触发真实网络请求，需要更强的失败隔离和可观测性。

## 当前推荐下一步

优先进入 M7 分享、导出和外部集成，同时补 M6+ 邮件操作增强。分享导出、本地只读 HTML、账号/项目池导出能继续贴近原项目工作流；IMAP 附件下载、HTML 安全渲染和远端失败重试属于下一批稳定性增强。
