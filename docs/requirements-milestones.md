...
- IMAP 支持 XOAUTH2 登录、LIST 文件夹发现和 Junk/Deleted/Trash/Spam 等特殊文件夹映射。
- 邮件详情支持全部附件打包为本地 ZIP 下载、已缓存原始 MIME raw 查看、附件下载中状态和错误提示。
- 设置页支持本地保留统计、邮件/临时消息/附件/导出清理入口、清理确认、操作中关闭确认和刷新缓存数量提示。
- 邮件列表、邮件详情、附件入口。
- 刷新日志和账号刷新状态记录。
...
- 已按批次提交并推送：
  ...
  - `feat: enhance imap auth and folder mapping`
  - `feat: add local retention management`
  - `feat: add automation observability dashboard`
  - `feat: add local mail share records`
  - `feat: add local appearance themes`
  - `test: add local desktop workflow coverage`
  - `feat: add confirmed account secret export`
...
- 当前 P1 项已完成，后续缺口从 P2 开始推进。

P2：后续增强

- 当前 P2 项已完成，后续按真实使用反馈继续优化。
...
- 当前桌面优先测试已覆盖本地 workflow，后续按真实缺陷补充更细测试。
...
- 已交付：本地邮件分享记录，支持创建本地 HTML 分享、过期时间、撤销记录和导出清理联动撤销。
- 已交付：本地外观主题，支持预设皮肤、强调色和设置持久化。
- 已交付：选中账号凭据 CSV 导出，要求本地密码和确认短语二次确认。
- 剩余增强：本地 HTTP API（暂缓）。
...
- 已交付：独立自动化仪表盘，展示任务汇总、错误分类、重试退避和转发通道熔断状态。
- 已交付：重试队列按错误类型计算退避和最大次数，转发通道近 30 分钟连续失败会进入软熔断。
- 已交付：本地 workflow 级测试覆盖账号导入、刷新失败入队、邮件操作、临时邮箱、备份恢复和重试队列。
...
- P9.6 Hardening and Documentation：已新增服务商故障排查与验收清单，README/架构文档已链接；provider 能力元数据、provider-specific 凭据错误归类、刷新失败处理建议和自动化回归覆盖记录已整理，仍需真实 Gmail/QQ/163 账号人工验收。

## 当前推荐下一步

继续按“桌面版够用，优先补功能”推进。当前桌面优先级中的 P0/P1/P2 项已补齐；M9 已完成首批 Provider Foundation、Gmail OAuth/API 同步基础、Gmail 远端操作、Gmail `historyId` 增量同步、QQ IMAP preset/提示基础、163 IMAP preset/中文文件夹映射基础、P9.5 Provider UX and Batch Operations 主要 UI 能力，以及 P9.6 故障排查/验收文档。下一步建议在具备真实 Gmail/QQ/163 测试账号时按 [`docs/provider-operations.md`](provider-operations.md) 补做人工验收；Web 服务和浏览器扩展暂不处理。
