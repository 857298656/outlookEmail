...
- 账号/分组 HTTP 代理链配置，Graph、IMAP HTTP CONNECT 和 HTTP 转发通道按主代理、备用代理顺序故障切换。
- 账号授权编辑面板。
...
- 代理配置和故障切换剩余项：SMTP 代理隧道适配；账号/分组代理字段 UI、继承/覆盖规则、Graph 请求、IMAP HTTP CONNECT 和 HTTP 转发通道备用代理重试已完成。
- 分组管理增强：编辑、删除、排序、三级树展示、跨层级移动、级联处理账号归属。
- 账号批量操作：批量删除、批量移动分组、批量改转发开关、批量打/移除标签、批量导出。
- Token 刷新管理桌面视图：失败账号列表、刷新统计、单账号重试、批量重试、刷新历史和停止长任务。
- 账号敏感信息查看保护：本地密码二次验证后显示账号密码、IMAP 密码和 token 预览，避免普通编辑误清空密钥。
...
- 剩余交付物：调度历史独立仪表盘、SMTP 代理隧道适配、端到端测试。
- 风险点：自动化任务会触发真实网络请求，需要更强的失败隔离和可观测性。

## 当前推荐下一步

继续按“桌面版够用，优先补功能”推进。下一批建议先补 SMTP 代理隧道适配，然后推进账号批量操作、Token 刷新管理桌面视图、临时邮箱增强；Web 服务和浏览器扩展暂不处理。
