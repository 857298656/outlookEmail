...
继续按“桌面版够用，优先补功能”推进。当前桌面优先级中的 P0/P1/P2 项已补齐；M9 已启动并完成首批 Provider Foundation、Gmail OAuth/API 同步基础、Gmail 远端操作、Gmail `historyId` 增量同步和 QQ IMAP preset/提示基础。下一步建议继续 P9.4：163 Mail IMAP Provider 的提示、文档和真实账号验收准备；同时在具备真实 Gmail/QQ 测试账号时补做人工验收。Web 服务和浏览器扩展暂不处理。
